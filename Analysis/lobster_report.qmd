...
lobster_traps <- read_csv(here::here("data", "Lobster_Trap_Counts_All_Years_20210519.csv")) %>%   mutate(TRAPS = na_if(TRAPS, -99999))
```
...
# Lobster Abundance and Size (Figures 1–3)
...
# Fishing Pressure Using Lobster Trap Counts (Figures 4–6)
...
# Save the plot as a PNG file in the figs folder
ggsave(
  filename = "../figs/distribution_of_trap_counts_by_year.png",  # file path and name
  plot = distribution_of_trap_counts_by_year,                   # plot object to save
...
lobster_traps_filtered <- lobster_traps %>%
  filter(SITE %in% c("AQUE", "CARP", "IVEE", "MOHK", "NAPL")) %>%
  mutate(SITE = factor(SITE, levels = sort(unique(SITE))))
# Step 2: summarize filtered data
lobster_traps_summarize <- lobster_traps_filtered %>%
  group_by(SITE, YEAR) %>%
...
# Save the plot as a PNG file in the figs folder
ggsave(
  filename = "../figs/lobster_traps_by_site_year.png",   # file path and name
  plot = lobster_traps_by_site_year,                    # plot object to save
  width = 10,                                          # width in inches
  height = 6,                                          # height in inches
  dpi = 300                                            # resolution
)
# Filter to match owner analysis sites
lobster_traps_filtered <- lobster_traps %>%
  filter(SITE %in% c("AQUE", "CARP", "IVEE", "MOHK", "NAPL")) %>%
  mutate(SITE = factor(SITE, levels = sort(unique(SITE))))
```
...
```{r}
# Create fishing pressure categories, filtered to partner sites
lobster_traps_fishing_pressure <- lobster_traps %>%
  filter(
    SITE %in% c("AQUE", "CARP", "IVEE", "MOHK", "NAPL"),
    YEAR %in% c(2019, 2020, 2021),
    !is.na(TRAPS)   # ← removes NA problem
  ) %>%
  mutate(
    SITE = factor(SITE, levels = c("AQUE", "CARP", "IVEE", "MOHK", "NAPL")),
    FISHING_PRESSURE = if_else(TRAPS >= 8, "high", "low")
  ) %>%
...
### system("git add Analysis/lobster_report.qmd Analysis/lobster_report.html Analysis/lobster_report_files")Lobster Abundance and Size (All Years)
